...
class ResourceBundleMessageResolverTest {
    ...
    void constructor_ShouldCreateResolver_ForValidInput() {
        assertThatCode(() -> new ResourceBundleMessageResolver("someBaseFileName")).doesNotThrowAnyException();
    }
    ...
    void resolve_ShouldReturnCorrectMessage_ForMessageArguments() {
        ...
        assertThat(result)
                .isEqualTo(
                        String.format("A Test Message with Argument: \"%s\" and \"%s\"",
                                firstArgument,
                                secondArgument)
                );
    }
    ...
    void resolve_ShouldReturnDefaultMessageWithArguments_ForUnresolvableKeyAndArguments() {
        ...
        var result = resolver.resolveOrDefault(invalidKey, "A Test Message with Argument: \"{0}\" and \"{1}\"", firstArgument, secondArgument);
    ...
    @Test
    void resolve_ShouldReturnDefaultMessageWithArgumentPlaceholders_ForUnresolvableKeyAndArgumentsLessThanRequired() {
        var invalidKey = "some.key";
        var firstArgument = "First Argument";

        var result = resolver.resolveOrDefault(invalidKey, "A Test Message with Argument: \"{0}\" and \"{1}\"", firstArgument);
    ...
    @Test
    void resolve_ShouldReturnDefaultMessageWithFirstArguments_ForUnresolvableKeyAndArgumentsMoreThanRequired() {
        var invalidKey = "some.key";
        var firstArgument = "First Argument";
        var secondArgument = "Second Argument";
        var thirdArgument = "Third Argument";

        var result = resolver.resolveOrDefault(invalidKey, "A Test Message with Argument: \"{0}\" and \"{1}\"", firstArgument, secondArgument, thirdArgument);
