 ...
 *
 * <p>This class looks up message patterns from resource bundles based on a configurable {@code
 * baseName} and the given {@link Locale}. Resolved messages can include parameter placeholders
 * (e.g. {@code {0}, {1}, ...}), which are formatted using {@link MessageFormat}.
 *
 * <h2>Caching</h2>
 *
 * <p>Resolved {@link ResourceBundle} instances are cached per locale in a thread-safe {@link
 * ConcurrentMap} to improve performance and avoid repeatedly loading the same bundles.
 *
 * <h2>Fallback behavior</h2>
 *
 * <ul>
 *   <li>If the message key cannot be found in the resource bundle, the {@code defaultMessage} will
 *       be returned if provided.
 *   <li>If no default message is given, the unresolved message key itself will be returned.
 * </ul>
 *
 * @author Jonas Nuber
 */
public class ResourceBundleMessageResolver implements MessageResolver {
   ...
   *
   * <p>The lookup process works as follows:
   *
   * <ol>
   *   <li>Attempt to load the {@link ResourceBundle} for the given locale (from cache if already
   *       loaded).
   *   <li>Retrieve the message pattern associated with the key.
   *   <li>Format the message with the provided arguments (if any).
   *   <li>If the key is missing, return the formatted {@code defaultMessage} (if provided),
  ...
  public String resolve(String key, List<Object> args, String defaultMessage, Locale locale) {
    ...
    try {
      ResourceBundle bundle =
          cache.computeIfAbsent(locale, loc -> ResourceBundle.getBundle(baseName, loc));
